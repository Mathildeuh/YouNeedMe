...
/**
 * The single storage abstraction point: swapped wholesale at startup based on {@code
 * storage.type} in {@code config.yml} (SQLite by default). Every backend (SQL over HikariCP,
 * MongoDB, or flat JSON) implements this same contract, so nothing above this layer - services,
 * commands, expansions - ever needs to know which one is active.
 *

...
public interface EconomyRepository {
     ...
     * Backends must serialize this per-{@code (player, currencyId)} pair (row-level lock / CAS
     * loop / single-writer actor - the implementation's choice) so concurrent deposits/withdrawals
     * for the same player can never race.
     */
    CompletableFuture<Double> applyDelta(UUID player, String currencyId, double delta, double defaultBalance);

    CompletableFuture<Void> setBalance(UUID player, String currencyId, double amount);

    CompletableFuture<List<EconomyService.BalanceEntry>> top(String currencyId, int offset, int limit);

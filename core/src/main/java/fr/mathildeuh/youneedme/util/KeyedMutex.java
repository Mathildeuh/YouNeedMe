...
/**
 * Serializes async actions per key so, e.g., two {@code /pay} commands spammed for the same player
 * can never race each other into a double-spend - this is the "verrouillage par joueur" the brief
 * calls for on economy/auction/shop transactions. Each action for a given key only starts once the
 * previous one for that same key has finished (success or failure); different keys run fully in
 * parallel.
 */

...
public final class SqlEconomyRepository implements EconomyRepository {
                ...
                connection -> {
                    try (PreparedStatement ps =
                            connection.prepareStatement(
                                    "SELECT balance FROM ynm_balances WHERE player = ? AND"
                                            + " currency_id = ?")) {
        ...
        return sql.submit(
                connection -> {
                    ...
                    try (PreparedStatement update =
                            connection.prepareStatement(
                                    "UPDATE ynm_balances SET balance = balance + ? WHERE player = ?"
                                            + " AND currency_id = ?")) {
        ...
        return sql.run(
                connection -> {
                    try (PreparedStatement ps =
                        ...
                        ps.setString(
                                6,
                                log.relatedPlayer() == null
                                        ? null
                                        : log.relatedPlayer().toString());
    ...
    public CompletableFuture<List<EconomyTransactionLog>> history(UUID player, int limit) {
        return sql.submit(
                connection -> {
                    try (PreparedStatement ps =
                            connection.prepareStatement(
                                    "SELECT * FROM ynm_economy_log WHERE player = ? ORDER BY ts"
                                            + " DESC LIMIT ?")) {
                        ps.setString(1, player.toString());
                        ps.setInt(2, limit);
                        try (ResultSet rs = ps.executeQuery()) {
    ...
    /**
     * Inserts the {@code (player, currencyId)} row with {@code defaultBalance} iff it doesn't exist
     * yet - a no-op otherwise.
     */
    private void ensureRowExists(
            Connection connection, UUID player, String currencyId, double defaultBalance)
            throws SQLException {
        String sqlText =
